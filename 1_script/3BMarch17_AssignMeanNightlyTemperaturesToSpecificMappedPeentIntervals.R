...
for (H in interval.length){
  ...
  Times<-read.csv(paste0("0_data/processed/3_PeentsMapped_SunAndMoon/SunMoonPeentDetections.",H,".int.csv"),header=TRUE)
  Times.temp<-merge(Times, MeanTemp, by=c("sm_id","date"), all.x=TRUE)
  print(paste0("Temperatures added to ",H," ARU intervals"))
  write.csv(Times.temp, file=paste0("0_data/processed/3_PeentsMapped_SunAndMoon/TempSunMoonPeentDetections.",H,".int.csv"))
