...
peents.pervisit<-peentsintD %>%
  group_by(sm_id, intervalsP) %>% 
  summarize(numdetect= sum(PEENTS))
write.csv(peents.pervisit, file="0_data/processed/9_PeentActivityRates_MixedModels/temp/peentspervisitMay13GAMMs.csv")
#varies from 0 to 567 peents/10 minute interval. 
#196/15824 intervals have >100 peents/interval.
#86/15824 intervals have >200 peents/interval.
#36/15824 intervals have >300 peents/interval.
#11/15824 intervals have >400 peents/interval.
#2/15824 intervals have >500 peents/interval.
...
peentsint$TSSS.s<-scale(peentsint$TSSS, center=FALSE, scale=TRUE)#scaled to reduce rank deficiency of GAMMs
...
peents.pervisit$latgroupN<-as.factor(ifelse(peents.pervisit$latitude>55,"N","S"))
...
full.GAM.SS <- gamm4(numdetect ~latgroupN + s(meantemp.s, k = 4, bs = "cs", by=latgroupN) + s(ordinal.s, k = 4, bs = "cs", by=latgroupN) + s(TSSS.s, k = 4, bs = "cs", by=latgroupN), random=~(1|sm_id), data=peents.pervisit, family=negbin(theta=thetaval, link="log"))
...
bs <- function(data, indices){
  ...
  try(full.GAM.SS <- gamm4(numdetect ~latgroupN + s(meantemp.s, k = 4, bs = "cs", by=latgroupN) + s(ordinal.s, k = 4, bs = "cs", by=latgroupN) + s(TSSS.s, k = 4, bs = "cs", by=latgroupN), random=~(1|sm_id), data=peents.pervisit, family=negbin(theta=thetaval, link="log")))
  #plot.gam(full.GAM.SS$gam, main = "Smoothed Untransformed Boom Activity Estimates")
...
save(peentresults, CoefB, CoefBCI50, CoefBCI90, file="3_output/data/4A_May_BootstrappedPeentGAMMs/boot20samples10minuteintPeents_M14.RData")
#save as csv
df<-data.frame(CoefB)
...
write.csv(df, file="3_output/data/4A_May_BootstrappedPeentGAMMs/boot20samples10minuteintPeents_M14.csv")
...
ggsave("3_output/figures/4A_May_BootstrappedPeentGAMMs/boot20visits10minuteintervalPeent_M14.jpeg", plot=GG, width=12, height=6, units=c("in"), dpi=300)
...
plotTSSSData<-peentsintD.train[order(peentsintD.train$TSSScorr),]#read.csv("0_data/processed/10B_BoomBootstraps/plotTSSSData.csv",header=TRUE)
plotOrdinalData<-peentsintD.train[order(peentsintD.train$julian),]#read.csv("0_data/processed/10B_BoomBootstraps/plotOrdinalData.csv",header=TRUE)
...
PredS <- t(apply(PredB.S, 1, quantile, c(0.5, 0.25, 0.75), na.rm=TRUE))
...
## calculate median and 50% CIs
PredN <- t(apply(PredB.N, 1, quantile, c(0.5, 0.25, 0.75), na.rm=TRUE))

## plot
tiff("3_output/figures/4A_May_BootstrappedPeentGAMMs/testGAMMmatplotTSSS_M14.tiff", units="in", width=8, height=8, res=300)
par(mfrow=c(2,2))
matplot(plotTSSSDataS$TSSScorr, 
        PredB.S, 
        type="l", 
        xlab="TSSS (Day 180)",
        ylab="Peents per 10 min Site 8246")
matplot(plotTSSSDataS$TSSScorr, 
        PredS, 
        type="l", 
        lty=c(1,2,2), 
...
matplot(plotTSSSDataN$TSSScorr, 
        ...
        type="l", 
        xlab="TSSS (Day 160)",
        ylab="Peents per 10 min Site 13588")
matplot(plotTSSSDataN$TSSScorr, 
        PredN, 
        type="l", 
        lty=c(1,2,2), 
...
## plot
tiff("3_output/figures/4A_May_BootstrappedPeentGAMMs/testGAMMmatplotOrdinal_M14.tiff", units="in", width=8, height=8, res=300)
par(mfrow=c(2,2))
...
plotTempData<-peentsintD.train[order(peentsintD.train$meantemp),]
...
## plot
tiff("3_output/figures/4A_May_BootstrappedPeentGAMMs/testGAMMmatplotTemperature_M14.tiff", units="in", width=8, height=8, res=300)
par(mfrow=c(2,2))
...
peents.pervisit.test$latgroupN<-as.factor(ifelse(peents.pervisit.test$latitude>55,"N","S"))
## model matrix that'll match the coefficients
...
str(TestDatapreds)
write.csv(TestDatapreds, file="3_output/data/4A_May_BootstrappedPeentGAMMs/TestDatapreds.csv")
...
#       50%         5%        95% 
#0.13488234 0.02502464 0.22827643 
